...
#define RMF_COPY_FRAME_D(                                             \
  ...
  void copy_node_frame_type_node(NodeConstHandle in,
    ...
    for (unsigned int i = 0; i < inch.size(); ++i) {
      copy_node_frame_type_node(inch[i], outch[i], inkeys, outkeys);
    }
  }
  ...
  void copy_node_frame_type(FileConstHandle in,
    ...
    for (unsigned int i = 0; i < incats.size(); ++i) {
      std::vector<Key<TypeTraits> > cinkeys =
  ...
  bool get_equal_node_structure(NodeConstHandle in,
    ...
    if (in.get_type() != out.get_type()) {
      if (print_diff) {
        std::cout << "Node types differ at " << in << " vs " << out
                  << std::endl;
      }
      ret = false;
    }
    if (in.get_name() != out.get_name()) {
      if (print_diff) {
        std::cout << "Node names differ at " << in << " vs " << out
                  << std::endl;
      }
      ret = false;
    }
...
#define RMF_EQUAL_FRAME_D(                                            \
      ...
      if (in.get_has_frame_value(inkeys[i]) !=
        ...
        if (print_diff) {
          std::cout << "Node differ about having value "
                    << in.get_file().get_name(inkeys[i]) << " at " << in
                    << " and " << out << std::endl;
        }
        ret = false;
      } else if (in.get_has_frame_value(inkeys[i]) &&
                 in.get_value(inkeys[i]) != out.get_value(outkeys[i])) {
        if (print_diff) {
          std::cout << "Node differ about value "
                    << in.get_file().get_name(inkeys[i]) << " at " << in
                    << " and " << out << " " << in.get_value(inkeys[i])
  ...
  bool get_equal_node_frame_type(FileConstHandle in,
    ...
    return get_equal_node_frame_type_node(
