...
RMF_ENABLE_WARNINGS namespace RMF {
  ...
  NodeID get_alternative_impl(NodeConstHandle cur, FloatKey base_resolution_key,
                              ...
                              RepresentationType type, float resolution) {
  ...
  NodeIDs get_alternatives_impl(NodeConstHandle cur,
                                FloatKey /*base_resolution_key*/,
                                IntsKey /*types_key*/, IntsKey roots_key,
                                FloatsKey /*resolutions_key*/) {
  ...
  void Alternatives::add_alternative(NodeHandle root, RepresentationType type) {
    get_node().get_shared_data()
        ->access_static_value(get_node().get_id(), types_key_).push_back(type);
    get_node().get_shared_data()
  ...
  NodeConstHandles AlternativesConst::get_alternatives(
      RepresentationType /*type*/) const {
    NodeConstHandles ret;
    RMF_FOREACH(NodeID id, get_alternatives_impl(
                               get_node(), base_resolution_key_, types_key_,
  ...
  float AlternativesConst::get_resolution(NodeID id) const {
    ...
    for (unsigned int i = 0; i < roots.size(); ++i) {
      if (static_cast<unsigned int>(roots[i]) == id.get_index()) {
  ...
  RepresentationType AlternativesConst::get_representation_type(
      NodeID id) const {
    if (id == get_node().get_id()) {
      return PARTICLE;
    }
