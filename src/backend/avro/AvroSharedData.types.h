...
#define RMF_AVRO_SHARED_TYPE(                                                  \
  ...
  const Ucname##DataIndexConstPair get_frame_type_data(                        \
      Key<Ucname##Traits>, int node, Category category, int frame) const {     \
  ...
  Ucname##DataIndexPair access_frame_type_data(                                \
      Key<Ucname##Traits>, int node, Category category, int frame) {           \
  ...
  void set_value_frame(                                                        \
      unsigned int frame, Key<Ucname##Traits> k, Ucname##Traits::Type v) {     \
  ...
  bool get_has_frame_value(unsigned int node, Key<Ucname##Traits> k) const {   \
    ...
    Ucname##DataIndexConstPair data =                                          \
        get_frame_type_data(k, node, cat, P::get_current_frame());             \
    return !Ucname##Traits::get_is_null_value(                                 \
        get_one_value(data.get<0>(), data.get<1>(), k));                       \
  }                                                                            \
  std::vector<Key<Ucname##Traits> > get_##lcname##_keys(Category cat) {        \
