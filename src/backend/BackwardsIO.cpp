...
namespace backends {
  namespace {
  boost::array<std::string, 3> make_array(std::string a, std::string b, std::string c) {
    boost::array<std::string, 3> ret;
    ret[0] = a;
    ret[1] = b;
    ret[2] = c;
    return ret;
  }
    boost::array<std::string, 4> make_array(std::string a, std::string b, std::string c, std::string d) {
    boost::array<std::string, 4> ret;
    ret[0] = a;
    ret[1] = b;
    ret[2] = c;
    ret[3] = d;
...
const P3 vector_3_names[] = {
  ...
  P3("rgb color", make_array("rgb color red", "rgb color green", "rbg color blue")),
  P3("force", make_array("force cartesian x", "force cartesian y", "force cartesian z")),
  P3("torque",
     make_array("torque cartesian x", "torque cartesian y", "torque cartesian z"))};
...
V3N vector_3_names_map(vector_3_names, vector_3_names + vector_3_names_size);
  ...
  typedef std::pair<std::string, boost::array<std::string, 4> > P4;

const P4 vector_4_names[] = {
  P4("orientation",
     make_array("orientation r", "orientation i", "orientation j", "orientation k")),
     P4("rotation",
	make_array("reference frame orientation r", "reference frame orientation i",
...
const P3 vectors_3_names[] =
  {P3("coordinates list", make_array("cartesian xs", "cartesian ys", "cartesian zs"))};
