...
struct TestSchema {
  ...
  void createExampleSchema() {
    ...
    rmf_avro::RecordSchema myRecord("complex");

    // Now populate my record with fields (each field is another schema):
    myRecord.addField("real", rmf_avro::DoubleSchema());
    myRecord.addField("imaginary", rmf_avro::DoubleSchema());

    // The complex record is the same as used above, let's make a schema
    // for an array of these record

    rmf_avro::ArraySchema complexArray(myRecord);

    rmf_avro::ValidSchema validComplexArray(complexArray);
  ...
  void buildSchema() {
    ...
    IntSchema intSchema;
    rmf_avro::MapSchema map = MapSchema(IntSchema());
...
struct TestBadStuff {
  void testBadFile() {
    ...
    std::string error;
    bool result = rmf_avro::compileJsonSchema(in, schema, error);
