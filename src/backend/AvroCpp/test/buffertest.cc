...
void TestSeek() {
  BOOST_MESSAGE("TestSeek");
  {
    ...
    rmf_avro::OutputBuffer tmp1, tmp2, tmp3;
...
void server(boost::barrier &b) {
  ...
  rmf_avro::InputBuffer::const_iterator iter = rbuf.begin();
  while (iter != rbuf.end()) {
...
void TestAsioBuffer() {
  ...
  BOOST_MESSAGE("TestAsioBuffer");
  {
    ...
    const rmf_avro::InputBuffer rbuf(buf);
...
void TestCopy() {
  ...
  // Test1, small data, small buffer
  {
    ...
    BufferReader br(wb);
    br.seek(5);
    rmf_avro::InputBuffer ib = br.copyData(msg.size() - 10);
  ...
  // Test2, small data, large buffer
  {
    ...
    rmf_avro::OutputBuffer wb(bufsize);
  ...
  // Test3 Border case, buffer is exactly full
  {
    ...
    rmf_avro::InputBuffer ib = br.copyData(20);
  ...
  // Test4, no data
  {
    ...
    try {
      BufferReader br(wb);
      rmf_avro::InputBuffer ib = br.copyData(10);
    }
    catch (std::exception &e) {
      cout << "Intentially triggered exception: " << e.what() << endl;
    }
...
void TestBug() {
  BOOST_MESSAGE("TestBug");
  {
    ...
    {
      rmf_avro::InputBuffer ib(rxBuf.extractData());
      buf.append(ib);
    }
