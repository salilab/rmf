...
template <>
struct codec_traits<PaddedRecord> {
...
struct codec_traits<Integer> {
  static void decode(Decoder &d, Integer &c) { rmf_avro::decode(d, c.re); }
};

template <>
struct codec_traits<Double> {
  static void decode(Decoder &d, Double &c) { rmf_avro::decode(d, c.re); }
};
}
...
class DataFileTest {
 ...
 public:
  ...
  void testReadDoubleTwoStep() {
    auto_ptr<rmf_avro::DataFileReaderBase> base(
        new rmf_avro::DataFileReaderBase(filename));
  ...
  void testSeeks() {
    ...
    {
      auto_ptr<avro::DataFileWriter<PaddedRecord> > writer(
          new avro::DataFileWriter<PaddedRecord>(filename, dschema));
    ...
    {
      auto_ptr<avro::DataFileReader<PaddedRecord> > reader(
          new avro::DataFileReader<PaddedRecord>(filename));
