...
struct TestCodeGenerator {
  ...
  void serializeToScreen() {
    std::cout << "Serialize:\n";
    rmf_avro::Writer writer;

    rmf_avro::serialize(writer, myRecord_);
  ...
  void serializeToScreenValid() {
    ...
    rmf_avro::ValidatingWriter writer(schema_);
  ...
  void testParser() {
    ...
    rmf_avro::serialize(s, myRecord_);

    testgen::RootRecord inRecord;
    rmf_avro::Reader p(s.buffer());
    rmf_avro::parse(p, inRecord);

    checkOk(myRecord_, inRecord);
  }

  void testParserValid() {
    rmf_avro::ValidatingWriter s(schema_);

    rmf_avro::serialize(s, myRecord_);

    testgen::RootRecord inRecord;
    rmf_avro::ValidatingReader p(schema_, s.buffer());
    rmf_avro::parse(p, inRecord);
  ...
  void testNameIndex() {
    const rmf_avro::NodePtr &node = schema_.root();
    size_t index = 0;
    ...
    const rmf_avro::NodePtr &enumNode = node->leafAt(index);
  ...
  TestCodeGenerator() {
    ...
    rmf_avro::compileJsonSchema(in, schema_);
  }

  testgen::RootRecord myRecord_;
  rmf_avro::ValidSchema schema_;
};
...
struct TestSchemaResolving {
  ...
  rmf_avro::InputBuffer serializeWriteRecordToBuffer() {
    std::ostringstream ostring;
    rmf_avro::Writer s;
    rmf_avro::serialize(s, writeRecord_);
    return s.buffer();
  }

  void parseData(const rmf_avro::InputBuffer &buf,
                 rmf_avro::ResolverSchema &xSchema) {
    rmf_avro::ResolvingReader r(xSchema, buf);

    rmf_avro::parse(r, readRecord_);
  }
  ...
  void test() {
    ...
    rmf_avro::ResolverSchema xSchema(writerSchema_, readerSchema_, layout);

    printRecord(writeRecord_);

    rmf_avro::InputBuffer buffer = serializeWriteRecordToBuffer();
  ...
  TestSchemaResolving() {
    ...
    rmf_avro::compileJsonSchema(win, writerSchema_);

    std::ifstream rin(gReader.c_str());
    rmf_avro::compileJsonSchema(rin, readerSchema_);
  }

  testgen::RootRecord writeRecord_;
  rmf_avro::ValidSchema writerSchema_;

  testgen2::RootRecord readRecord_;
  rmf_avro::ValidSchema readerSchema_;
};
