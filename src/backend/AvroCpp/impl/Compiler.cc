...
const map<std::string, Entity>::const_iterator findField(
    const Entity &e, const map<std::string, Entity> &m,
...
static Name getName(const Entity &e, const map<std::string, Entity> &m,
                    const std::string &ns) {
...
static NodePtr makeNode(const Entity &e, const map<std::string, Entity> &m,
  ...
  if (NodePtr result = makePrimitive(type)) {
    if (m.size() > 1) {
      throw Exception(boost::format("Unknown additional Json fields: %1%") %
                      e.toString());
    } else {
      return result;
    }
