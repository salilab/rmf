...
void DataFileWriterBase::sync() {
  ...
  rmf_avro::encode(*encoderPtr_, objectCount_);
  int64_t byteCount = buffer_->byteCount();
  rmf_avro::encode(*encoderPtr_, byteCount);
  encoderPtr_->flush();
  ...
  encoderPtr_->init(*stream_);
  rmf_avro::encode(*encoderPtr_, sync_);
  encoderPtr_->flush();
...
void DataFileWriterBase::writeHeader() {
  encoderPtr_->init(*stream_);
  rmf_avro::encode(*encoderPtr_, magic);
  rmf_avro::encode(*encoderPtr_, metadata_);
  rmf_avro::encode(*encoderPtr_, sync_);
  encoderPtr_->flush();
}
...
bool DataFileReaderBase::readDataBlock() {
  ...
  stream_->backup(n);
  rmf_avro::decode(*decoder_, objectCount_);
  int64_t byteCount;
  rmf_avro::decode(*decoder_, byteCount);
  decoder_->init(*stream_);
...
void DataFileReaderBase::seekBlockBytes(int64_t offset) {
  ...
  if (offset == blockOffset_) {
    ...
    throw Exception(
        "Cannot seek backwards in streams. This might be made to "
        "work in some cases.");
  }
}
