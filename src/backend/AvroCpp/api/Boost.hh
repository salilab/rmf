...
#ifndef rmf_avro_Boost_hh__
#define rmf_avro_Boost_hh__
...
#include <vector>
// this implements a minimal subset of ptr_vector (the parts of the API used by
// avro)
namespace boost {
template <class T>
class ptr_vector {
 public:
  ...
  void push_back(T *v) { ptrs_.push_back(v); }
  void pop_back() {
  ...
  const T &at(size_t index) const { return *(ptrs_.at(index)); }
  const T &operator[](size_t index) const { return *(ptrs_[index]); }
  size_t size() const { return ptrs_.size(); }
  void reserve(size_t elems) { ptrs_.reserve(elems); }
