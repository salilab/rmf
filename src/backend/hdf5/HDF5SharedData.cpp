...
void HDF5SharedData::open_things(bool create, bool read_only) {
  ...
  if (create) {
    ...
    if (read_only) {
      ...
      file_ = HDF5::open_hdf5_file_read_only_returning_nonconst(get_file_path());
    } else {
      file_ = HDF5::open_hdf5_file(get_file_path());
    }
    std::string version;
...
#define RMF_LIST_KEYS(lcname, Ucname, PassValue, ReturnValue,           \
  ...
  for (int pf = 0; pf < 2; ++pf) {                                      \
    ...
    for (unsigned int j = 0; j < sz[0]; ++j) {                          \
      std::string name = nameds.get_value(HDF5::HDF5DataSetIndexD<1>(j));     \
      int id;                                                           \
...
void HDF5SharedData::initialize_free_nodes() {
  HDF5::HDF5DataSetIndexD<2> dim = node_data_[0].get_size();
  for (unsigned int i = 0; i < dim[0]; ++i) {
    if (IndexTraits::
        get_is_null_value(node_data_[0].get_value(HDF5::HDF5DataSetIndexD<2>(i,
                                                                       0)))) {
...
void HDF5SharedData::initialize_categories() {
  ...
  HDF5::HDF5DataSetIndex1D sz = category_names_.get_size();
  for (unsigned int i = 0; i < sz[0]; ++i) {
    std::string name = category_names_.get_value(HDF5::HDF5DataSetIndex1D(i));
    Category cat(i);
...
int HDF5SharedData::add_node(std::string name, unsigned int type) {
  int ret;
  if (free_ids_.empty()) {
    HDF5::HDF5DataSetIndexD<1> nsz = node_names_.get_size();
    ret = nsz[0];
    ++nsz[0];
    node_names_.set_size(nsz);
    HDF5::HDF5DataSetIndexD<2> dsz = node_data_[0].get_size();
    dsz[0] = ret + 1;
  ...
  node_names_.set_value(HDF5::HDF5DataSetIndexD<1>(ret), name);
  node_data_[0].set_value(HDF5::HDF5DataSetIndexD<2>(ret, TYPE), type);
  node_data_[0].set_value(HDF5::HDF5DataSetIndexD<2>(ret, CHILD),
                          IndexTraits::get_null_value());
  node_data_[0].set_value(HDF5::HDF5DataSetIndexD<2>(ret, SIBLING),
                          IndexTraits::get_null_value());
  return ret;
}
int HDF5SharedData::get_first_child(unsigned int node) const {
    check_node(node);
  return node_data_[0].get_value(HDF5::HDF5DataSetIndexD<2>(node, CHILD));
}
int HDF5SharedData::get_sibling(unsigned int node) const {
    check_node(node);
  return node_data_[0].get_value(HDF5::HDF5DataSetIndexD<2>(node, SIBLING));
}
void HDF5SharedData::set_first_child(unsigned int node, int c) {
    check_node(node);
  return node_data_[0].set_value(HDF5::HDF5DataSetIndexD<2>(node, CHILD), c);
}
void HDF5SharedData::set_sibling(unsigned int node, int c) {
    check_node(node);
  return node_data_[0].set_value(HDF5::HDF5DataSetIndexD<2>(node, SIBLING), c);
}
std::string HDF5SharedData::get_name(unsigned int node) const {
  if (node < get_number_of_real_nodes()) {
    check_node(node);
    return node_names_.get_value(HDF5::HDF5DataSetIndexD<1>(node));
  } else {
    return "bond";
  }
}
unsigned int HDF5SharedData::get_type(unsigned int index) const {
  if (index < get_number_of_real_nodes()) {
    check_node(index);
    return node_data_[0].get_value(HDF5::HDF5DataSetIndexD<2>(index, TYPE));
  } else {
    return BOND;
  }
}
...
unsigned int HDF5SharedData::get_number_of_sets(int arity) const {
  ...
  unsigned int ct = 0;
  for (unsigned int i = 0; i < sz[0]; ++i) {
    if (node_data_[arity - 1].get_value(HDF5::HDF5DataSetIndexD<2>(i, 0)) >= 0) {
      ++ct;
    }
  }
  return ct;
...
unsigned int HDF5SharedData::get_set_member(int arity, unsigned int index,
  ...
  return node_data_[arity - 1].get_value(HDF5::HDF5DataSetIndexD<2>(index,
                                                              member_index + 1));
}
unsigned int HDF5SharedData::add_category_impl(std::string name) {
  // fill in later
  int sz = category_names_.get_size()[0];
  category_names_.set_size(HDF5::HDF5DataSetIndex1D(sz + 1));
  category_names_.set_value(HDF5::HDF5DataSetIndex1D(sz), name);
  return sz;
}
...
void HDF5SharedData::set_frame_name(int i, std::string str) {
  ...
  if (static_cast<int>(frame_names_.get_size()[0]) <= i) {
    frame_names_.set_size(HDF5::HDF5DataSetIndexD<1>(i + 1));
  }
  frame_names_.set_value(HDF5::HDF5DataSetIndexD<1>(i), str);
}
std::string HDF5SharedData::get_frame_name(int i) const {
  RMF_USAGE_CHECK(i != ALL_FRAMES,
                  "The static data frame does not have a name");
  if (static_cast<int>(frame_names_.get_size()[0]) > i) {
    return frame_names_.get_value(HDF5::HDF5DataSetIndexD<1>(i));
  } else {
    return std::string();
  }
}
