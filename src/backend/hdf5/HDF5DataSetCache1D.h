...
class HDF5DataSetCacheD<TypeTraits, 1>/*: public boost::noncopyable*/ {
  typedef HDF5::HDF5DataSetD<typename TypeTraits::HDF5Traits, 1> DS;
  ...
  void initialize(DS ds) {
    ...
    ds_ = ds;
    HDF5::HDF5DataSetIndexD<1> sz = ds_.get_size();
    HDF5::HDF5DataSetIndexD<1> lb(0);
    if (sz != lb) {
      if (TypeTraits::HDF5Traits::BatchOperations) {
        ...
        for (unsigned int i = 0; i < cache_.size(); ++i) {
          cache_[i] = ds_.get_value(HDF5::HDF5DataSetIndexD<1>(i));
        }
      }
    }
    dirty_begin_ = sz[0];
...
public:
  ...
  void flush() {
    ...
    if (ds_.get_size()[0] != cache_.size()) {
      ds_.set_size(HDF5::HDF5DataSetIndexD<1>(cache_.size()));
    }
    if (TypeTraits::HDF5Traits::BatchOperations) {
      HDF5::HDF5DataSetIndexD<1> sz(dirty_end_ - dirty_begin_);
      HDF5::HDF5DataSetIndexD<1> lb(dirty_begin_);
      ...
      for ( int i = dirty_begin_; i < dirty_end_; ++i) {
        ds_.set_value(HDF5::HDF5DataSetIndexD<1>(i), cache_[i]);
      }
    }
    dirty_begin_ = ds_.get_size()[0];
    dirty_end_ = -1;
  }
  void set_size(const HDF5::HDF5DataSetIndexD<1> &ijk) {
    RMF_INTERNAL_CHECK(!name_.empty(),
                       "Name never set");
    if (ds_ == DS()) {
      HDF5::HDF5DataSetCreationPropertiesD<typename TypeTraits::HDF5Traits, 1> props;
      props.set_chunk_size(HDF5::HDF5DataSetIndexD<1>(256));
  ...
  void set_value(const HDF5::HDF5DataSetIndexD<1> &ijk,
  ...
  typename TypeTraits::Type get_value(const HDF5::HDF5DataSetIndexD<1> &ijk) const {
    return cache_[ijk[0]];
  }
  HDF5::HDF5DataSetIndexD<1> get_size() const {
    return HDF5::HDF5DataSetIndexD<1>(cache_.size());
  }
};
