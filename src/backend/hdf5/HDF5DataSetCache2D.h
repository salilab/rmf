...
class HDF5DataSetCacheD<TypeTraits, 2>/*: public boost::noncopyable*/ {
  ...
  void initialize(DS ds) {
    ...
    HDF5::HDF5DataSetIndexD<2> lb(0, 0);
    if (extents_ != lb) {
      if (TypeTraits::HDF5Traits::BatchOperations) {
        ...
        for (unsigned int i = 0; i < extents_[0]; ++i) {
          for (unsigned int j = 0; j < extents_[1]; ++j) {
            cache_[i][j] = get_as<typename TypeTraits::Type>(ds_.get_value(HDF5::HDF5DataSetIndexD<2>(i, j)));
          }
        }
      }
    }
...
public:
  ...
  void set(HDF5::HDF5Group parent, std::string name) {
    ...
    if (parent_.get_has_child(name_)) {
      ...
      extents_ = HDF5::HDF5DataSetIndexD<2>(0, 0);
    }
  }
  void reset() {
    flush();
  ...
  void flush() {
    ...
    if (TypeTraits::HDF5Traits::BatchOperations) {
      ...
      for (unsigned int i = 0; i < extents_[0]; ++i) {
        for (unsigned int j = 0; j < extents_[1]; ++j) {
          ds_.set_value(HDF5::HDF5DataSetIndexD<2>(i, j),
  ...
  void set_size(const HDF5::HDF5DataSetIndexD<2> &ijk) {
    RMF_INTERNAL_CHECK(!name_.empty(),
                       "Name never set");
    if (ds_ == DS()) {
      HDF5::HDF5DataSetCreationPropertiesD<typename TypeTraits::HDF5Traits, 2> props;
      props.set_chunk_size(HDF5::HDF5DataSetIndexD<2>(256, 4));
  ...
  void set_value(const HDF5::HDF5DataSetIndexD<2> &ijk,
                 typename TypeTraits::Type  value) {
    cache_[ijk[0]][ijk[1]] = value;
    dirty_ = true;
  }
  typename TypeTraits::Type get_value(const HDF5::HDF5DataSetIndexD<2> &ijk) const {
    typename TypeTraits::Type ret = cache_[ijk[0]][ijk[1]];
    return ret;
  }
  HDF5::HDF5DataSetIndexD<2> get_size() const {
    return extents_;
  }
};
