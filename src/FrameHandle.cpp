...
FrameHandle FrameHandle::add_child(std::string name, FrameType t) {
  FrameHandle ret(get_shared_data()->add_child(get_frame_id(), name, t),
                  get_shared_data());
...
void FrameHandle::add_child(FrameConstHandle nh) {
  get_shared_data()->add_child(get_frame_id(), nh.get_frame_id());
}
...
FrameHandles FrameHandle::get_children() const {
  FrameIDs children = get_shared_data()->get_children(get_frame_id());
