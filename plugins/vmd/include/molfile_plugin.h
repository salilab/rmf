...
typedef struct {
  ...
  int num_basis_funcs; /**< number of uncontracted basis functions in basis
                          array */
...
typedef struct {
  ...
  double *
      nuc_charge; /**< array(natom) containing the nuclear charge of atom i */
...
typedef struct {
  ...
  int *angular_momentum; /**< 3 ints per wave function coefficient do describe
                          ...
                          *   E.g. S={0 0 0}, Px={1 0 0}, Dxy={1 1 0}, or
                          * Fyyz={0 2 1}.
                          */
...
typedef struct {
  double *carthessian; /**< hessian matrix in cartesian coordinates
                        * (ncart)*(ncart)
                        *   as a single array of doubles (row(1),
                        * ...,row(natoms)) */
...
typedef struct molfile_qm_timestep_metadata {
  ...
  int num_orbitals_per_wavef[MOLFILE_MAXWAVEPERTS]; /**< # orbitals for each
                                                       wavefunction */
...
typedef struct {
  ...
  vmdplugin_HEAD
       ...
       * The comma separated list will be expanded when filename extension
       * matching
       * is performed.  If multiple plugins solicit the same filename
       * extensions,
       * the one that lists the extension earliest in its list is selected. In
       * the
 ...
 */
   ...
   */
  int (*read_angles)(void *handle, int *numangles, int **angles,
                     int **angletypes, int *numangletypes,
                     char ***angletypenames, int *numdihedrals, int **dihedrals,
                     int **dihedraltypes, int *numdihedraltypes,
                     char ***dihedraltypenames, int *numimpropers,
                     int **impropers, int **impropertypes,
                     int *numimpropertypes, char ***impropertypenames,
                     int *numcterms, int **cterms, int *ctermcols,
                     int *ctermrows);
   ...
   * (Cross terms pertain to the CHARMM/NAMD CMAP feature)
   */
  int (*write_angles)(void *handle, int numangles, const int *angles,
   ...
   *  as a 2-D matrix)
   */
  int (*read_angles)(void *, int *numangles, int **angles, double **angleforces,
                     int *numdihedrals, int **dihedrals,
                     double **dihedralforces, int *numimpropers,
                     int **impropers, double **improperforces, int *numcterms,
   ...
   *  as a 2-D matrix)
   */
  int (*write_angles)(void *, int numangles, const int *angles,
  ...
  /**
    ...
    */
  molfile_ssize_t (*read_times)(void *, molfile_ssize_t start,
                                molfile_ssize_t count, double *times);
#endif
