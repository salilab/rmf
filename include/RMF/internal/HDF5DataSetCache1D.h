...
namespace RMF {
  ...
  namespace internal {
    ...
    template <>
    class HDF5DataSetCacheD<StringTraits, 1>: public boost::noncopyable {
      typedef HDF5DataSetD<StringTraits, 1> DS;
      typedef typename StringTraits::Types array_type;
      array_type cache_;
      bool dirty_;
      DS ds_;
      // to create on demand
    ...
    public:
      ...
      void set(HDF5Group parent, std::string name) {
        ...
        name_=name;
        if (parent_.get_has_child(name_)) {
          initialize(parent_.get_child_data_set<StringTraits, 1>(name_));
        }
      }
      void reset() {
        flush();
      ...
      void set_size(const HDF5DataSetIndexD<1> &ijk) {
        RMF_INTERNAL_CHECK(!name_.empty(),
                           "Name never set");
        if (ds_== DS()) {
          ds_= parent_.add_child_data_set<StringTraits, 1>(name_);
        } else {
          flush();
        }
        ds_.set_size(ijk);
      ...
      void set_value(const HDF5DataSetIndexD<1> &ijk,
                     typename StringTraits::Type value) {
        RMF_USAGE_CHECK(ijk[0] < cache_.size(),
                        "Not big enough");
        cache_[ijk[0]]=value;
        dirty_=true;
