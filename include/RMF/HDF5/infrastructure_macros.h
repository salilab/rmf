...
#define RMF_TRAITS_ONE(UCName, UCNames, lcname, index, hdf5_disk, hdf5_memory, \
  ...
  struct RMFEXPORT UCName##Traits : public internal::BaseTraits<               \
    ...
    static UCName read_value_dataset(hid_t d, hid_t is, hid_t sp) {            \
      UCName ret;                                                              \
      rv_ds;                                                                   \
      return ret;                                                              \
    }                                                                          \
    static void write_values_dataset(hid_t d, hid_t is, hid_t s,               \
                                     const UCNames& v) {                       \
      if (v.empty()) return;                                                   \
      wvs_ds;                                                                  \
    }                                                                          \
    static UCNames read_values_dataset(hid_t d, hid_t is, hid_t sp,            \
                                       unsigned int sz) {                      \
    ...
    static hid_t get_hdf5_fill_type() { return hdf5_fill; }                    \
    static const UCName& get_fill_value() { return get_null_value(); }         \
...
#define RMF_TRAITS(UCName, UCNames, lcname, index, hdf5_disk, hdf5_memory,     \
  ...
  struct UCNames##Traits : public internal::BaseTraits<                        \
    ...
    static hid_t get_hdf5_memory_type() {                                      \
      static RMF_HDF5_HANDLE(                                                  \
          ints_type, H5Tvlen_create(UCName##Traits::get_hdf5_memory_type()),   \
          H5Tclose);                                                           \
      return ints_type;                                                        \
    }                                                                          \
    static void write_value_dataset(hid_t d, hid_t is, hid_t s,                \
                                    const UCNames& v) {                        \
      hvl_t data;                                                              \
      data.len = v.size();                                                     \
      if (data.len > 0) {                                                      \
      ...
      RMF_HDF5_CALL(                                                           \
          H5Dwrite(d, get_hdf5_memory_type(), is, s, H5P_DEFAULT, &data));     \
    }                                                                          \
    static UCNames read_value_dataset(hid_t d, hid_t is, hid_t sp) {           \
      hvl_t data;                                                              \
      H5Dread(d, get_hdf5_memory_type(), is, sp, H5P_DEFAULT, &data);          \
      UCNames ret(data.len);                                                   \
      std::copy(static_cast<UCName*>(data.p),                                  \
                static_cast<UCName*>(data.p) + data.len, ret.begin());         \
      free(data.p);                                                            \
      return ret;                                                              \
    }                                                                          \
    static void write_values_dataset(hid_t d, hid_t is, hid_t s,               \
...
#define RMF_SIMPLE_TRAITS(UCName, UCNames, lcname, index, hdf5_disk,           \
  ...
  RMF_TRAITS(UCName, UCNames, lcname, index, hdf5_disk, hdf5_memory,           \
             hdf5_fill, avro_type, null_value, get_is_null_value(i),           \
             RMF_HDF5_CALL(                                                    \
                 H5Dwrite(d, get_hdf5_memory_type(), is, s, H5P_DEFAULT, &v)), \
             RMF_HDF5_CALL(H5Dread(d, get_hdf5_memory_type(), is, sp,          \
                                   H5P_DEFAULT, &ret)),                        \
             RMF_HDF5_CALL(H5Dwrite(d, get_hdf5_memory_type(), is, s,          \
                                    H5P_DEFAULT, const_cast<UCName*>(&v[0]))), \
             RMF_HDF5_CALL(H5Dread(d, get_hdf5_memory_type(), is, sp,          \
                                   H5P_DEFAULT, &ret[0])),                     \
             RMF_HDF5_CALL(H5Awrite(a, get_hdf5_memory_type(), &v[0])),        \
             RMF_HDF5_CALL(H5Aread(a, get_hdf5_memory_type(), &ret[0])), true)

#else

#define RMF_TRAITS_ONE(UCName, UCNames, lcname, index, hdf5_disk, hdf5_memory, \
                       hdf5_fill, avro_value, null_value, null_test, wv_ds,    \
                       rv_ds, wvs_ds, rvs_ds, wvs_a, rvs_a, multiple)          \
...
#define RMF_TRAITS(UCName, UCNames, lcname, index, hdf5_disk, hdf5_memory, \
                   hdf5_fill, avro_traits, null_value, null_test, wv_ds,   \
                   rv_ds, wvs_ds, rvs_ds, wvs_a, rvs_a, batch)             \
  struct UCName##Traits {                                                  \
    typedef UCName Type;                                                   \
    typedef UCNames Types;                                                 \
