...
#define RMF_TRAITS_ONE(UCName,                                              \
  ...
  struct RMFEXPORT UCName##Traits :                                         \
      public internal::                                                     \
          BaseTraits<UCName, UCNames, avro_type, index, multiple> {         \
    ...
    static void write_values_dataset(hid_t d,                               \
                                     hid_t is,                              \
                                     hid_t s,                               \
                                     const UCNames& v) {                    \
      if (v.empty())                                                        \
    ...
    static UCNames read_values_dataset(hid_t d,                             \
                                       hid_t is,                            \
                                       hid_t sp,                            \
                                       unsigned int sz) {                   \
...
#define RMF_TRAITS(UCName,                                                     \
  ...
  RMF_TRAITS_ONE(UCName,                                                       \
                 ...
                 rvs_a,                                                        \
                 batch);                                                       \
  struct UCNames##Traits : public internal::BaseTraits<UCNames,                \
                                                       std::vector<UCNames>,   \
    ...
    static hid_t get_hdf5_disk_type() {                                        \
      static RMF_HDF5_HANDLE(                                                  \
          ints_type,                                                           \
          H5Tvlen_create(UCName##Traits::get_hdf5_disk_type()),                \
          H5Tclose);                                                           \
      return ints_type;                                                        \
    }                                                                          \
    static hid_t get_hdf5_memory_type() {                                      \
      static RMF_HDF5_HANDLE(                                                  \
          ints_type,                                                           \
          H5Tvlen_create(UCName##Traits::get_hdf5_memory_type()),              \
          H5Tclose);                                                           \
      return ints_type;                                                        \
    }                                                                          \
    static void write_value_dataset(hid_t d,                                   \
                                    hid_t is,                                  \
                                    hid_t s,                                   \
                                    const UCNames& v) {                        \
      hvl_t data;                                                              \
    ...
    ;                                                                          \
    static std::vector<UCNames> read_values_dataset(hid_t d,                   \
                                                    hid_t is,                  \
                                                    hid_t sp,                  \
                                                    unsigned int sz) {         \
      RMF_UNUSED(d);                                                           \
    ...
    template <class O> static bool get_is_null_value(const O& o) {             \
      return o.empty();                                                        \
    }                                                                          \
...
/** Declare a type traits*/
#define RMF_SIMPLE_TRAITS(UCName,                                              \
                          UCNames,                                             \
                          lcname,                                              \
                          index,                                               \
                          hdf5_disk,                                           \
  ...
  RMF_TRAITS(UCName,                                                           \
             ...
             RMF_HDF5_CALL(H5Dread(                                            \
                 d, get_hdf5_memory_type(), is, sp, H5P_DEFAULT, &ret)),       \
             ...
             RMF_HDF5_CALL(H5Dread(                                            \
                 d, get_hdf5_memory_type(), is, sp, H5P_DEFAULT, &ret[0])),    \
