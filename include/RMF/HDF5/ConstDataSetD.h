  ...
  class ConstDataSetD : public ConstDataSetAttributes {
   ...
   protected:
    ...
    ConstDataSetD(boost::shared_ptr<SharedHandle> parent,
      ...
      RMF_USAGE_CHECK(
          H5Sget_simple_extent_ndims(sel) == D,
    ...
    hsize_t* get_ones() const { return data_->ones_; }
    const Handle& get_row_data_space() const { return data_->rds_; }
    const Handle& get_data_space() const { return data_->sel_; }
    const Handle& get_input_data_space() const { return data_->ids_; }
    void check_index(const DataSetIndexD<D>& ijk) const {
      DataSetIndexD<D> sz = get_size();
      for (unsigned int i = 0; i < D; ++i) {
        RMF_USAGE_CHECK(ijk[i] < sz[i],
                        RMF::internal::get_error_message(
                            "Index is out of range: ", ijk[i], " >= ", sz[i]));
      }
    }
    void initialize_handles() {
      ...
      RMF_HDF5_CALL(H5Sget_simple_extent_dims(
          get_data_space(), data_->size_.begin(), NULL));
    }
   ...
   public:
    ...
    RMF_SHOWABLE(ConstDataSetD, "ConstDataSet" << D << "D " << P::get_name());
#ifndef SWIG
...
#ifndef RMF_DOXYGEN
#define RMF_DECLARE_CONST_DATA_SET(                                    \
